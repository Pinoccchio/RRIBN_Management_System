...
export default function AdminAnnouncementsPage() {
  ...
  const fetchAllCompanies = async () => {
    try {
      ...
      if (data.success && data.data) {
        ...
        logger.success(`Fetched ${data.data.length} companies`, { context: 'AdminAnnouncementsPage' });
      }
    } catch (error) {
  ...
  // Fetch all announcements (battalion-wide)
  const fetchAllAnnouncements = async () => {
    try {
      setIsLoading(true);
      logger.info('Fetching announcements for admin...', { context: 'AdminAnnouncementsPage' });

      const response = await fetch('/api/admin/announcements?status=all&limit=1000');
      const data = await response.json();

      if (data.success) {
        setAllAnnouncements(data.data);
        logger.success(`Fetched ${data.data.length} announcements`, { context: 'AdminAnnouncementsPage' });
      } else {
        logger.error('Failed to fetch announcements', data.error, { context: 'AdminAnnouncementsPage' });
      }
    } catch (error) {
      logger.error('Error fetching announcements', error, { context: 'AdminAnnouncementsPage' });
    } finally {
      setIsLoading(false);
    }
  };
  ...
  useEffect(() => {
    ...
    if (searchQuery.trim()) {
      ...
      filtered = filtered.filter((announcement) => {
        ...
        return title.includes(query) || content.includes(query) || creatorName.includes(query);
      });
    }
  ...
  const handleCreate = async (input: CreateAnnouncementInput) => {
    try {
      logger.info('Creating announcement (admin)', { title: input.title });
      ...
      if (data.success) {
        ...
        setToast({
          message: input.publish_now ? 'Announcement published successfully!' : 'Announcement saved as draft!',
          type: 'success',
        });
        fetchAllAnnouncements();
      } else {
  ...
  const handleUpdate = async (announcementId: string, input: UpdateAnnouncementInput) => {
    try {
      logger.info('Updating announcement (admin)', { announcementId });
  ...
  const handleDeleteAnnouncement = async (announcementId: string) => {
    try {
      logger.info('Deleting announcement (admin)', { announcementId });
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Announcements"
        description="Battalion-wide announcement management for all companies"
        breadcrumbs={[{ label: 'Dashboard', href: '/admin' }, { label: 'Announcements' }]}
        action={
          <Button variant="primary" onClick={() => setCreateModalOpen(true)} leftIcon={<Plus className="w-5 h-5" />}>
            Create Announcement
          </Button>
        }
      />
