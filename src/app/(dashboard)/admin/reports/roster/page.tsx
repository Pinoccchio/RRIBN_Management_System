...
export default function AdminRosterReportPage() {
  ...
  const fetchRosterData = async () => {
    try {
      setLoading(true);
      const response = await fetch('/api/admin/reports/roster');
  ...
  return (
    <div>
      <PageHeader
        title="Battalion Roster Report"
        description="Complete personnel roster across all companies"
        breadcrumbs={[
      ...
      <div className="mb-6 flex justify-between items-center">
            ...
            {companies.length > 0 ? (
              <>
                ...
                <span className="font-semibold text-navy-900">
                  {companies.join(', ')} ({data.length} reservists)
                </span>
              </>
            ) : (
              <span className="font-semibold text-navy-900">
                All Companies ({data.length} reservists)
              </span>
            )}
          </p>
        ...
        <ExportPDFButton
          reportTitle="Battalion Roster Report"
          reportSubtitle="Personnel listing for all companies"
          company={companies.length > 0 ? companies.join(', ') : 'All Companies'}
          columns={pdfColumns}
          data={data}
          filename={generateFileName('Battalion_Roster', 'All')}
        />
      </div>
      ...
      <ReportTable
        ...
        data={data}
        loading={loading}
        emptyMessage="No reservists found in the battalion"
      />
    </div>
  );
}
