 ...
 *
 * Mark attendance for reservists in a training session (admin can mark for any training)
 *
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!account || (account.role !== 'admin' && account.role !== 'super_admin')) {
      logger.error('Non-admin user attempted to mark attendance', {
        userId: user.id,
        role: account?.role,
      });
    ...
    logger.error('Unexpected error in POST /api/admin/training/[id]/attendance', error);
