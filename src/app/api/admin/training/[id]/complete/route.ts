 ...
 *
 * Award training hours and complete a training session (admin can complete any training)
 *
 ...
 *
 * Admin Role: DATA COLLECTION for battalion-wide promotion decision making
 *
...
export async function POST(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!account || (account.role !== 'admin' && account.role !== 'super_admin')) {
      logger.error('Non-admin user attempted to complete training', {
        userId: user.id,
        role: account?.role,
      });
    ...
    logger.error('Unexpected error in POST /api/admin/training/[id]/complete', error);
