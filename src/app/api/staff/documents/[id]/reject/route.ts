...
import type { RejectDocumentInput } from '@/lib/types/document';
...
export async function PUT(
  ...
  logger.separator();
  logger.info('API Request', { context: `PUT /api/staff/documents/${params.id}/reject` });
  ...
  try {
    ...
    // Parse request body
    const body: RejectDocumentInput = await request.json();
    ...
    // Update document
    logger.dbQuery('UPDATE', 'documents', `Rejecting document: ${params.id}`);
    ...
    await supabase
      ...
      .insert({
        ...
        message: `Your ${document.document_type} document has been rejected. Reason: ${body.rejection_reason}`,
        type: 'document',
        reference_id: document.id,
        reference_table: 'documents',
      });

    logger.success(`Successfully rejected document: ${params.id}`);
    logger.separator();

    return NextResponse.json({ success: true, message: 'Document rejected successfully' });
  } catch (error) {
