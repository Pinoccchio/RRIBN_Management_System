...
export async function PUT(request: NextRequest, { params }: { params: { id: string; entryId: string } }) {
  try {
    ...
    return NextResponse.json({ success: true, data, message: 'Award entry updated successfully' });
  } catch (error) {
...
export async function DELETE(request: NextRequest, { params }: { params: { id: string; entryId: string } }) {
  try {
    ...
    if (error) return NextResponse.json({ success: false, error: error.message }, { status: 500 });
    return NextResponse.json({ success: true, message: 'Award entry deleted successfully' });
  } catch (error) {
